...
export const getKeyMetrics = async (query: string) => {
  try {
    const data = await axios.get<CompanyKeyMetrics[]>(
      `https://financialmodelingprep.com/stable/key-metrics-ttm?symbol=AAPL&apikey=${process.env.REACT_APP_API_KEY}`
      //${query}&apikey=${process.env.REACT_APP_API_KEY}`
    );
    return data;
  } catch (error: any) {
    console.log("error message: ", error.message);
...
export const getCompData = async (query: string) => {
  try {
    /*const data = await axios.get<CompanyCompData[]>(
      `https://financialmodelingprep.com/stable/peers-bulk?symbol=${query}&apikey=${process.env.REACT_APP_API_KEY}`
    );*/
    const data : CompanyCompData = 
    {
      symbol: query,
      peersList: ["MSFT", "GOOGL", "AMZN", "FB", "TSLA"],
    };
    return data;
  } catch (error: any) {
    console.log("error message: ", error.message);
...
export const getTenK = async (query: string) => {
  try {
    const data = await axios.get<CompanyTenK[]>(
      `https://financialmodelingprep.com/stable/sec-filings-company-search/name?company=${query}&apikey=${process.env.REACT_APP_API_KEY}`
    );
    return data;
  } catch (error: any) {
    console.log("error message: ", error.message);
